// Tablero — brand and marketing imagery (logo + decorative stock photos used
// on the public/marketing pages). Menu, loyalty and ops data now come from the
// backend; this file only holds static brand assets.

export const IMG = {
  logo: '/brand/simpleLogo.png',
  logoMark: '/brand/logoMark.png', // tightly-cropped, transparent mark for nav/footer
  texture: '/brand/texture.png',
  interior: 'https://images.pexels.com/photos/35505245/pexels-photo-35505245.jpeg',
  interior2: 'https://images.pexels.com/photos/29692582/pexels-photo-29692582.jpeg',
  chef: 'https://images.pexels.com/photos/32742915/pexels-photo-32742915.jpeg',
};
