...
export default function WaiterPickup() {
  ...
  React.useEffect(() => {
    ...
    conn.on('OrderReadyForPackaging', () => { load(); toast.info(t('waiter.readyAlert')); });
  ...
  const collect = async (id) => {
    try {
      ...
      toast.success(t('waiter.collected'));
      load();
    } catch (err) {
      toast.error(err?.response?.data?.message || t('waiter.collectFail'));
    }
  };
  ...
  const handOver = async (id) => {
    try {
      ...
      toast.success(t('waiter.handedOver'));
      load();
    } catch (err) {
      toast.error(err?.response?.data?.message || t('waiter.handOverFail'));
    }
  };
  ...
  return (
    <OpsLayout title={t('waiter.title')} subtitle={t('waiter.subtitle')} right={right}>
      {loading ? (
        <div className="py-20 flex justify-center"><Loader2 className="animate-spin text-ink-muted" /></div>
      ) : orders.length === 0 ? (
        <div className="py-20 text-center text-ink-muted">{t('waiter.none')}</div>
      ) : (
          ...
          {orders.map(o => {
            ...
            return (
              <div key={o.id} data-testid={`waiter-pickup-${o.id}`} className="bg-white border border-border rounded-xl p-4">
                ...
                <div className="text-sm text-ink-muted mt-1">{new Date(o.pickupTime).toLocaleTimeString()}</div>
                ...
                <div className="mt-3 flex justify-between text-sm border-t border-border pt-3">
                  <span className="text-ink-muted">{t('waiter.balance')}</span>
                ...
                <div className="mt-3 flex gap-2">
                  {balanceDue && (
                    <button onClick={() => collect(o.id)}
                      ...
                      <HandCoins size={14} /> {t('waiter.collectBalance')}
                    </button>
                  )}
                  <button onClick={() => handOver(o.id)}
                    ...
                    <Package size={14} /> {t('waiter.handOver')}
                  </button>
                </div>
              </div>
            );
